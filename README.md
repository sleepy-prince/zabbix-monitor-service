...
Данный проект представляет собой локальный отказоустойчивый стек мониторинга на базе **Zabbix 7.0 LTS** [4]. Стек развернут в контейнерах Docker и настроен на сбор метрик хост-машины под управлением Linux в безопасном активном режиме.

## Архитектура решения

1. **Zabbix Database (PostgreSQL 17):** Хранилище данных, изолированное внутри внутренней сети Docker. Снабжено автоматической проверкой здоровья (`healthcheck`).
2. **Zabbix Server:** Главный вычислительный движок. Ожидает готовности базы данных перед запуском.
3. **Zabbix Web UI (Nginx):** Панель управления на порту.
